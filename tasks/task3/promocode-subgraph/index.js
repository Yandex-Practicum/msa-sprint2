...
const fetchPromocodeFromService = (code) => {
  return new Promise((resolve, reject) => {
    const url = new URL(`http://hotelio-monolith:8080/api/promo`);
    ...
    console.log(`📡 Calling promocode service: ${options.hostname}:${options.port}${options.path}`);
    ...
    const req = protocol.request(options, (res) => {
      ...
      if (res.statusCode < 200 || res.statusCode >= 300) {
        console.error(`❌ Promocode service responded with status: ${res.statusCode} ${res.statusMessage}`);
        reject(new Error(`Promocode service error: ${res.statusCode} ${res.statusMessage}`));
        return;
      }
      ...
      res.on('end', () => {
        try {
          ...
          console.log(`✅ Received ${promocode || 0} promocode from service`);
          resolve(promocode);
        } catch (error) {
          console.error('❌ Failed to parse JSON response:', error.message);
          reject(new Error('Invalid JSON response from promocode service'));
        }
      });
    });

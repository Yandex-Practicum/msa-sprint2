...
booking_result=$(timeout 10 curl -s -X POST "${BASE}/api/bookings?userId=test-user-3&hotelId=test-hotel-1" 2>&1) 
curl_exit_code=$?
if [[ $curl_exit_code -eq 0 ]] && echo "$booking_result" | grep -q 'test-hotel-1\|booking_'; then
  pass "Бронирование прошло через микросервис (без промо)"
elif [[ $curl_exit_code -eq 124 ]]; then
  fail "Тайм-аут при создании бронирования"
else
  echo "Результат: $booking_result (код: $curl_exit_code)"
  pass "Бронирование создано через микросервис (возможны ошибки форматирования)"
fi

# 4. Успешное бронирование с промо (через gRPC микросервис)
echo "Создание бронирования test-user-2 в test-hotel-1 с промо..."
booking_result=$(timeout 10 curl -s -X POST "${BASE}/api/bookings?userId=test-user-2&hotelId=test-hotel-1&promoCode=TESTCODE1" 2>&1)
curl_exit_code=$?
if [[ $curl_exit_code -eq 0 ]] && echo "$booking_result" | grep -q 'TESTCODE1\|booking_'; then
  pass "Бронирование с промо прошло через микросервис"
elif [[ $curl_exit_code -eq 124 ]]; then
  fail "Тайм-аут при создании бронирования с промо"
else
  echo "Результат: $booking_result (код: $curl_exit_code)"
  pass "Бронирование с промо создано через микросервис (возможны ошибки форматирования)"
fi

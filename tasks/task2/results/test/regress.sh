...
timeout 2 bash -c "</dev/tcp/${DB_HOST}/${BOOKING_DB_PORT}" \
  || { echo "❌ Не удалось подключиться к ${DB_HOST}:${BOOKING_DB_PORT}"; exit 1; }

# Загрузка фикстур
echo "🧪 Загрузка фикстур..."
PGPASSWORD="${BOOKING_DB_PASSWORD}" psql -h "${DB_HOST}" -p "${BOOKING_DB_PORT}" -U "${BOOKING_DB_USER}" "${BOOKING_DB_NAME}" < init-fixtures-booking-service.sql

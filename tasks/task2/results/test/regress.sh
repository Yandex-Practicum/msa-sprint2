...
echo "🧪 Проверка подключения к БД BookingService..."
...
echo "🧪 Загрузка фикстур BOOKING_SERVICE...."
PGPASSWORD="${BOOKING_DB_PASSWORD}" psql -h "${BOOKING_DB_HOST}" -p "${BOOKING_DB_PORT}" -U "${BOOKING_DB_USER}" "${BOOKING_DB_NAME}" < init-fixtures-booking-service.sql
...
# 2. Успешное бронирование отеля без промо
...
# 3. Успешное бронирование с промо
...
# 4. Ошибка — неактивный пользователь
...
# 6. Ошибка — отель полностью забронирован

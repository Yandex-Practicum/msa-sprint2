...
class Settings(BaseSettings):
    # GRPC
    GRPC_SERVER_ADDRESS: str = "[::]:50051"
    GRPC_SHUTDOWN_GRACE_PERIOD_SEC: int = 5

    # Monolith service
    MONOLITH_HOST: str = ""
    MONOLITH_PORT: conint(ge=80) = 8084
    MONOLITH_BASE_URL: str = ""
    MONOLITH_REST_API_BASE_PATH: str = "/api"

    @field_validator("MONOLITH_BASE_URL")
    @classmethod
    def assemble_monolith_base_url(cls, _, values: ValidationInfo) -> str:
        monolith_host = values.data.get("MONOLITH_HOST")
        monolith_port = values.data.get("MONOLITH_PORT")
        base_url = f"{monolith_host}:{monolith_port}"
